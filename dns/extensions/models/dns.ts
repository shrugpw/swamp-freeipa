/**
 * FreeIPA DNS management model.
 *
 * Connects to an existing FreeIPA server's JSON-RPC API to inspect and manage
 * DNS zones and records. `zoneFind`/`zoneShow`/`recordFind` snapshot state
 * read-only; `ensureZone`/`ensureRecords`/`ensureForwardZone` create objects
 * idempotently and `zoneDel`/`recordDel` remove them. `ensureRecords` is a
 * fan-out method — it takes a list of records and adds them all in one call,
 * recording what landed even on a partial failure. Every mutation records an
 * honest `attempt` audit resource on BOTH the success and failure paths (see
 * {@link recordAttempt}), the state resources (`zone`/`zones`/`record`/
 * `records`) are written only on success, and the destructive `zoneDel`/
 * `recordDel` are gated behind an explicit `confirm` flag plus a `live`
 * pre-flight existence check. This is the DNS surface of the
 * `@shrug/freeipa/*` family.
 *
 * ## Transport
 *
 * All network access is isolated in {@link ipaLogin}, the single transport seam
 * (the analog of `@shrug/vyos`'s `sshExec`). It performs a password
 * session-login against `/ipa/session/login_password`, keeps the `ipa_session`
 * cookie, and exposes a `call()` closure that issues JSON-RPC requests against
 * `/ipa/session/json`. A later revision can swap the auth step to Kerberos
 * (SPNEGO) behind the same seam without touching any method logic.
 *
 * ## Authentication & TLS
 *
 * The login password is a global argument — resolve it from a swamp vault at
 * model-create time with a CEL expression, e.g.
 * `--global-arg 'password=${{ vault.get("freeipa", "ADMIN_PASSWORD") }}'`.
 *
 * FreeIPA presents a cert issued by its own CA. Set the `caCert` global argument
 * to the PEM path of that CA (`/etc/ipa/ca.crt`) and the model trusts it for
 * just this connection — no system-trust changes required. Omit `caCert` to fall
 * back to the host's system trust store (which an enrolled host already has).
 *
 * @module
 */
import { z } from "npm:zod@4";

/** Connection and authentication settings for the target FreeIPA server. */
const GlobalArgsSchema = z.object({
  server: z
    .string()
    .describe("FreeIPA server FQDN, e.g. ipa1.example.com"),
  user: z
    .string()
    .default("admin")
    .describe("Principal username used for the session login"),
  password: z
    .string()
    .meta({ sensitive: true })
    .describe(
      "Password for the login principal — resolve from a vault via CEL, do not hard-code",
    ),
  apiVersion: z
    .string()
    .default("2.254")
    .describe("IPA API version pinned in each request's options"),
  caCert: z
    .string()
    .optional()
    .describe(
      "Path to the IPA CA cert (PEM, e.g. /etc/ipa/ca.crt) to trust for this connection; omit to use system trust",
    ),
});

type GlobalArgs = z.infer<typeof GlobalArgsSchema>;

/**
 * Unwrap FreeIPA's single-element attribute arrays.
 *
 * IPA returns most LDAP attributes as one-element arrays (`cn: ["host"]`). This
 * returns the first element of an array, or the value itself otherwise.
 *
 * @param v A raw attribute value from an IPA result.
 * @returns The scalar value, or `undefined` when absent/empty.
 */
export function one(v: unknown): unknown {
  if (Array.isArray(v)) return v.length > 0 ? v[0] : undefined;
  return v;
}

/**
 * Coerce a (possibly array-wrapped, possibly string) IPA value to an integer.
 *
 * @param v A raw attribute value.
 * @returns The parsed integer, or `null` when absent/non-numeric.
 */
export function toInt(v: unknown): number | null {
  const s = one(v);
  if (s === undefined || s === null) return null;
  const n = Number(s);
  return Number.isFinite(n) ? n : null;
}

/**
 * Coerce a raw IPA attribute value to an array of strings.
 *
 * @param v A raw attribute value (scalar, array, or absent).
 * @returns A string array (empty when absent).
 */
export function toStrArray(v: unknown): string[] {
  if (v === undefined || v === null) return [];
  return (Array.isArray(v) ? v : [v]).map((x) => String(x));
}

/**
 * Build a FreeIPA JSON-RPC request body.
 *
 * IPA's JSON-RPC shape is `{ method: "<cmd>/1", params: [positional, options],
 * id }` where `options` always carries the pinned API `version`.
 *
 * @param method IPA command name (e.g. `server_find`), without the `/1` suffix.
 * @param args Positional arguments in command order.
 * @param options Command options/flags (e.g. `{ all: true }`).
 * @param version API version to pin in the options object.
 * @returns The request body object ready to `JSON.stringify`.
 */
export function buildRpcBody(
  method: string,
  args: unknown[],
  options: Record<string, unknown>,
  version: string,
): {
  method: string;
  params: [unknown[], Record<string, unknown>];
  id: number;
} {
  return {
    method: `${method}/1`,
    params: [args, { ...options, version }],
    id: 0,
  };
}

/** A logged-in IPA JSON-RPC client. */
interface IpaClient {
  /**
   * Issue one JSON-RPC command.
   *
   * @param method IPA command name without the `/1` suffix.
   * @param args Positional arguments.
   * @param options Command options.
   * @returns The command's `result` object (`{ result, count, summary, … }`).
   */
  call: (
    method: string,
    args?: unknown[],
    options?: Record<string, unknown>,
  ) => Promise<Record<string, unknown>>;
}

/**
 * Log in to FreeIPA and return a JSON-RPC client — the sole transport seam.
 *
 * Performs a form password login to `/ipa/session/login_password`, captures the
 * `ipa_session` cookie, and returns a `call()` closure bound to that session.
 * TLS validation uses the host system trust store (see the module docs).
 *
 * TODO(kerberos): add a SPNEGO login path against `/ipa/session/login_kerberos`
 * selected by a global argument, keeping this return shape so methods are
 * unaffected.
 *
 * @param cfg Connection + credentials.
 * @returns A client whose `call()` issues authenticated JSON-RPC requests.
 * @throws If the login request fails or returns a non-2xx status.
 */
async function ipaLogin(cfg: GlobalArgs): Promise<IpaClient> {
  const base = `https://${cfg.server}/ipa`;
  const referer = `${base}/`;

  // Trust the IPA CA for just this connection when caCert is set (no
  // system-trust changes). Left open for the lifetime of the short-lived
  // method run; the runtime reclaims it on exit.
  const clientOpt: { client?: Deno.HttpClient } = {};
  if (cfg.caCert) {
    const ca = await Deno.readTextFile(cfg.caCert);
    clientOpt.client = Deno.createHttpClient({ caCerts: [ca] });
  }

  const loginResp = await fetch(`${base}/session/login_password`, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      "Accept": "text/plain",
      "Referer": referer,
    },
    body: new URLSearchParams({ user: cfg.user, password: cfg.password }),
    ...clientOpt,
  });
  // Drain the body so the connection can be reused/closed cleanly.
  const loginText = await loginResp.text();
  if (!loginResp.ok) {
    const reason = loginResp.headers.get("x-ipa-rejection-reason") ??
      loginText.slice(0, 200);
    throw new Error(
      `IPA login failed on ${cfg.server} (HTTP ${loginResp.status}): ${reason}`,
    );
  }

  const cookies = loginResp.headers.getSetCookie();
  const sessionCookie = cookies
    .map((c) => c.split(";", 1)[0])
    .find((c) => c.startsWith("ipa_session="));
  if (!sessionCookie) {
    throw new Error(
      `IPA login on ${cfg.server} returned no ipa_session cookie`,
    );
  }

  const call = async (
    method: string,
    args: unknown[] = [],
    options: Record<string, unknown> = {},
  ): Promise<Record<string, unknown>> => {
    const resp = await fetch(`${base}/session/json`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "Referer": referer,
        "Cookie": sessionCookie,
      },
      body: JSON.stringify(
        buildRpcBody(method, args, options, cfg.apiVersion),
      ),
      ...clientOpt,
    });
    const json = await resp.json() as {
      error: { name?: string; message?: string; code?: number } | null;
      result: Record<string, unknown> | null;
    };
    if (json.error) {
      throw new Error(
        `IPA ${method} failed: ${json.error.name ?? "Error"}: ` +
          `${json.error.message ?? "unknown"} (code ${json.error.code ?? "?"})`,
      );
    }
    if (!json.result) {
      throw new Error(`IPA ${method} returned no result`);
    }
    return json.result;
  };

  return { call };
}

// ---------------------------------------------------------------------------
// Shared write-kernel — CANONICAL across @shrug/freeipa/{user,group,cert}.
// Replicated verbatim in each package (extensions publish independently and
// cannot cross-import). Keep byte-identical; changes are a 3-repo sync.
// ---------------------------------------------------------------------------

/**
 * An honest record of one mutation attempt — the audit resource every write
 * method persists, on BOTH the success and failure paths.
 *
 * This is deliberately NOT a state resource: nobody CEL-references it to read a
 * live attribute, and when `success` is false it says so truthfully. That is
 * why persisting it on failure does not violate swamp's "don't persist
 * misleading state" rule — an attempt-log is telemetry-as-data, not state.
 */
const AttemptSchema = z.object({
  method: z.string().describe(
    "Model method name, e.g. user_add / ensureVlanGroup",
  ),
  ipaCommands: z.array(z.string()).describe("IPA commands attempted, in order"),
  request: z
    .record(z.string(), z.unknown())
    .describe("Sanitized method arguments (no secrets)"),
  success: z.boolean(),
  response: z
    .record(z.string(), z.unknown())
    .nullable()
    .describe("IPA result on success, null on failure"),
  error: z.string().nullable().describe(
    "Error message on failure, null on success",
  ),
  target: z.string().describe("Principal/group/cert acted on"),
  server: z.string(),
  attemptedAt: z.iso.datetime(),
});

/** One mutation attempt-record (see {@link AttemptSchema}). */
export type Attempt = z.infer<typeof AttemptSchema>;

/**
 * The structural slice of swamp's LogTape logger the write-kernel and methods
 * use. Declared locally (not imported) so the kernel stays self-contained and
 * byte-identical across the family. Messages use `{name}` placeholders filled
 * from the properties object — never string interpolation, and never secrets.
 */
interface MethodLogger {
  debug(message: string, properties?: Record<string, unknown>): void;
  info(message: string, properties?: Record<string, unknown>): void;
  warning(message: string, properties?: Record<string, unknown>): void;
  error(message: string, properties?: Record<string, unknown>): void;
}

/** Minimal execute context the write-kernel relies on. */
interface ExecuteContext {
  globalArgs: GlobalArgs;
  logger: MethodLogger;
  writeResource: (
    specName: string,
    name: string,
    data: Record<string, unknown>,
  ) => Promise<{ name: string }>;
  readResource?: (
    instanceName: string,
    version?: number,
  ) => Promise<Record<string, unknown> | null>;
}

/** Return shape for every method's execute function. */
interface ExecuteResult {
  dataHandles: Array<{ name: string }>;
}

/**
 * Run one mutation, persisting an `attempt` audit resource on BOTH paths, then
 * rethrowing on failure so the workflow step is marked failed.
 *
 * The three-way persistence rule (see the package README):
 *  - STATE resources (the object itself) are written only by the CALLER, only
 *    on success — never here.
 *  - IRREPLACEABLE material generated mid-op (a private key, a signed cert) is
 *    persisted by the CALLER the instant it is real, BEFORE calling this or
 *    before any later throw — never lose it to a downstream error.
 *  - The AUDIT record is this function's job, on both success and failure.
 *
 * @param context Execute context (needs writeResource + globalArgs).
 * @param target Principal/group/cert name acted on (for the audit + instance id).
 * @param method Model method name (audit `method`, e.g. "user_add").
 * @param ipaCommands IPA commands this attempt issues, in order.
 * @param request Sanitized, secret-free copy of the method arguments.
 * @param fn The mutation itself; its resolved value is recorded as `response`.
 * @returns Whatever `fn` resolves to, on success.
 * @throws Rethrows `fn`'s error after the failure attempt is persisted.
 */
async function recordAttempt<T extends Record<string, unknown>>(
  context: ExecuteContext,
  target: string,
  method: string,
  ipaCommands: string[],
  request: Record<string, unknown>,
  fn: () => Promise<T>,
): Promise<{ result: T; handle: { name: string } }> {
  const base = {
    method,
    ipaCommands,
    request,
    target,
    server: context.globalArgs.server,
    attemptedAt: new Date().toISOString(),
  };
  const instance = `attempt-${method}-${target}`;
  context.logger.info(
    "{method}: applying to {target} on {server}",
    { method, target, server: base.server, ipaCommands },
  );
  try {
    const result = await fn();
    const handle = await context.writeResource("attempt", instance, {
      ...base,
      success: true,
      response: result,
      error: null,
    });
    context.logger.info("{method}: succeeded on {target}", { method, target });
    return { result, handle };
  } catch (e) {
    await context.writeResource("attempt", instance, {
      ...base,
      success: false,
      response: null,
      error: e instanceof Error ? e.message : String(e),
    });
    context.logger.error(
      "{method}: failed on {target}: {error}",
      { method, target, error: e instanceof Error ? e.message : String(e) },
    );
    throw e;
  }
}

// ---------------------------------------------------------------------------
// DNS value-shaping — parse IPA dnszone / dnsrecord entries into friendly rows.
// ---------------------------------------------------------------------------

/**
 * Coerce a raw IPA boolean-ish value to a boolean.
 *
 * IPA renders booleans as a native `true`/`false` or as the strings
 * `"TRUE"`/`"FALSE"`, sometimes wrapped in a single-element array. Absence
 * means false — an unset `nsaccountlock`, for example, is an enabled account.
 *
 * @param v A raw attribute value.
 * @returns The boolean value; false when absent.
 */
export function toBool(v: unknown): boolean {
  const s = one(v);
  if (s === undefined || s === null) return false;
  if (typeof s === "boolean") return s;
  return String(s).toLowerCase() === "true";
}

/**
 * Map a DNS record type name to its IPA `dnsrecord_add`/`dnsrecord_del` option
 * key. IPA names per-type attributes `<type>record` (lowercase), e.g. an `A`
 * record is carried in `arecord`, `AAAA` in `aaaarecord`, `CNAME` in
 * `cnamerecord`. Known types are mapped explicitly; any other type falls back
 * to the `<type.toLowerCase()>record` convention so uncommon types still work.
 */
const RECORD_ATTR: Record<string, string> = {
  A: "arecord",
  AAAA: "aaaarecord",
  CNAME: "cnamerecord",
  PTR: "ptrrecord",
  NS: "nsrecord",
  MX: "mxrecord",
  TXT: "txtrecord",
  SRV: "srvrecord",
  SSHFP: "sshfprecord",
  TLSA: "tlsarecord",
};

/**
 * The IPA option key for a DNS record type (see {@link RECORD_ATTR}).
 *
 * @param type A DNS record type (case-insensitive), e.g. `A`, `aaaa`, `CNAME`.
 * @returns The IPA attribute name, e.g. `arecord`.
 */
export function recordAttr(type: string): string {
  return RECORD_ATTR[type.toUpperCase()] ?? `${type.toLowerCase()}record`;
}

/** A parsed FreeIPA DNS zone row (single-element IPA arrays unwrapped). */
const ZoneRowSchema = z.object({
  idnsname: z.string().describe(
    "Zone name (IPA `idnsname`), e.g. example.com.",
  ),
  active: z.boolean().describe(
    "Zone active state (IPA `idnszoneactive`); false == disabled",
  ),
  forwarders: z.array(z.string()).describe(
    "Per-zone forwarders (IPA `idnsforwarders`); set on forward zones",
  ),
  forwardPolicy: z.string().optional().describe(
    "Forward policy (IPA `idnsforwardpolicy`), e.g. `only`/`first`/`none`",
  ),
  raw: z.record(z.string(), z.unknown()).describe(
    "Full IPA zone entry, unmodified — nothing is lost",
  ),
});

/** A parsed FreeIPA DNS record row (single-element IPA arrays unwrapped). */
const RecordRowSchema = z.object({
  idnsname: z.string().describe(
    "Record name relative to the zone (IPA `idnsname`), e.g. `www`",
  ),
  ttl: z.number().nullable().describe("Record TTL in seconds (IPA `dnsttl`)"),
  aRecords: z.array(z.string()).describe("A records (IPA `arecord`)"),
  aaaaRecords: z.array(z.string()).describe("AAAA records (IPA `aaaarecord`)"),
  cnameRecords: z.array(z.string()).describe(
    "CNAME records (IPA `cnamerecord`)",
  ),
  raw: z.record(z.string(), z.unknown()).describe(
    "Full IPA record entry, unmodified — every rdata type is preserved here",
  ),
});

/** A single parsed zone + provenance — the `zone` state resource. */
const ZoneSchema = z.object({
  server: z.string(),
  zone: ZoneRowSchema,
  retrievedAt: z.iso.datetime(),
});

/** A snapshot of many zones — the `zones` state resource. */
const ZonesSchema = z.object({
  server: z.string(),
  zones: z.array(ZoneRowSchema),
  retrievedAt: z.iso.datetime(),
});

/** A single parsed record + provenance — the `record` state resource. */
const RecordSchema = z.object({
  server: z.string(),
  zone: z.string().describe("The zone this record belongs to"),
  record: RecordRowSchema,
  retrievedAt: z.iso.datetime(),
});

/**
 * A snapshot of a zone's records — the `records` state resource. `complete` is
 * `true` for a plain `recordFind` and for a fully-applied `ensureRecords`
 * batch; `ensureRecords` writes it `false` when only some records of the batch
 * landed before a hard failure, so a partial apply is honestly auditable.
 */
const RecordsSchema = z.object({
  server: z.string(),
  zone: z.string().describe("The zone these records belong to"),
  records: z.array(RecordRowSchema),
  complete: z.boolean().describe(
    "true == the full requested set is present; false == a partial ensureRecords apply",
  ),
  retrievedAt: z.iso.datetime(),
});

/**
 * Unwrap FreeIPA's DNS-name wire form. IPA returns dnszone/dnsrecord names — and
 * name-valued rdata such as CNAME/NS/PTR targets — as
 * `{ "__dns_name__": "fqdn." }` objects rather than plain strings. Reduce such an
 * object to its string; pass a plain string (or absence) through unchanged.
 */
export function dnsNameStr(v: unknown): string {
  if (v && typeof v === "object" && "__dns_name__" in v) {
    return String((v as { __dns_name__: unknown }).__dns_name__ ?? "");
  }
  return v == null ? "" : String(v);
}

/** A single DNS name: unwrap the first element (IPA wraps scalars in arrays). */
export function toDnsName(v: unknown): string {
  return dnsNameStr(one(v));
}

/**
 * An array of DNS names (e.g. CNAME/NS targets), each `__dns_name__`-unwrapped
 * and non-empty. Mirrors {@link toStrArray} but for name-valued rdata.
 */
export function toDnsNameArray(v: unknown): string[] {
  if (v == null) return [];
  const arr = Array.isArray(v) ? v : [v];
  return arr.map(dnsNameStr).filter((s) => s.length > 0);
}

/**
 * Map a raw IPA zone entry (a `dnszone_find`/`dnszone_show`/`dnszone_add` or
 * `dnsforwardzone_*` result row) into a friendly {@link ZoneRowSchema} row,
 * keeping the untouched entry under `raw` so no attribute is ever lost.
 *
 * @param entry A single zone entry from an IPA result.
 * @returns The flattened zone row.
 */
export function parseZone(
  entry: Record<string, unknown>,
): z.infer<typeof ZoneRowSchema> {
  return {
    idnsname: toDnsName(entry.idnsname),
    active: toBool(entry.idnszoneactive),
    forwarders: toStrArray(entry.idnsforwarders),
    forwardPolicy: one(entry.idnsforwardpolicy) as string | undefined,
    raw: entry,
  };
}

/**
 * Map a raw IPA record entry (a `dnsrecord_find`/`dnsrecord_show`/
 * `dnsrecord_add` result row) into a friendly {@link RecordRowSchema} row,
 * keeping the untouched entry under `raw` so every rdata type is preserved.
 *
 * @param entry A single record entry from an IPA result.
 * @returns The flattened record row.
 */
export function parseRecord(
  entry: Record<string, unknown>,
): z.infer<typeof RecordRowSchema> {
  return {
    idnsname: toDnsName(entry.idnsname),
    ttl: toInt(entry.dnsttl),
    aRecords: toStrArray(entry.arecord),
    aaaaRecords: toStrArray(entry.aaaarecord),
    cnameRecords: toDnsNameArray(entry.cnamerecord),
    raw: entry,
  };
}

/** A parsed zone row produced by {@link parseZone}. */
export type ZoneRow = z.infer<typeof ZoneRowSchema>;
/** A parsed record row produced by {@link parseRecord}. */
export type RecordRow = z.infer<typeof RecordRowSchema>;
/** A single-zone snapshot resource. */
export type Zone = z.infer<typeof ZoneSchema>;
/** A multi-zone snapshot resource. */
export type Zones = z.infer<typeof ZonesSchema>;
/** A single-record snapshot resource. */
export type Record_ = z.infer<typeof RecordSchema>;
/** A multi-record snapshot resource. */
export type Records = z.infer<typeof RecordsSchema>;

/** One record to ensure via {@link model}'s `ensureRecords` fan-out method. */
const RecordSpecSchema = z.object({
  name: z.string().describe(
    "Record name relative to the zone (e.g. `www`, or `@` for the apex)",
  ),
  type: z.string().describe("Record type, e.g. `A`, `AAAA`, `CNAME`"),
  data: z.string().describe(
    "Record rdata, e.g. an address `192.0.2.10` or a target `www.example.com.`",
  ),
  ttl: z.number().int().optional().describe("Optional TTL in seconds"),
});

/** A single desired record for the `ensureRecords` fan-out. */
export type RecordSpec = z.infer<typeof RecordSpecSchema>;

/**
 * Predicate: is this error IPA's "entry already exists" (`DuplicateEntry`,
 * code 4002)? Used to make `add` idempotent — a duplicate means the user is
 * already present, which is success for an idempotent create.
 *
 * Accepts the raw IPA error object, an `Error` whose message was formatted by
 * {@link ipaLogin} (`... DuplicateEntry: ... (code 4002)`), or a bare string.
 * Copied verbatim from the family's `group` package (extensions publish
 * independently and cannot cross-import); keep the two in sync.
 *
 * @param e The caught error (any shape).
 * @returns `true` when it represents a duplicate-entry condition.
 */
export function isDuplicateEntry(e: unknown): boolean {
  const matches = (s: string) =>
    /DuplicateEntry/i.test(s) || /\bcode 4002\b/.test(s);
  if (e && typeof e === "object") {
    const obj = e as { name?: unknown; code?: unknown; message?: unknown };
    if (obj.name === "DuplicateEntry") return true;
    if (obj.code === 4002) return true;
    if (typeof obj.message === "string" && matches(obj.message)) return true;
    return false;
  }
  if (typeof e === "string") return matches(e);
  return false;
}

/**
 * Predicate: is this error IPA's "no such entry" (`NotFound`, code 4001)? The
 * mirror of {@link isDuplicateEntry} — used to make `del` idempotent, where a
 * missing target means it is already gone, which is success for an idempotent
 * delete.
 *
 * Accepts the raw IPA error object, an `Error` whose message was formatted by
 * {@link ipaLogin} (`... NotFound: ... (code 4001)`), or a bare string.
 *
 * @param e The caught error (any shape).
 * @returns `true` when it represents a not-found condition.
 */
export function isNotFound(e: unknown): boolean {
  const matches = (s: string) => /NotFound/i.test(s) || /\bcode 4001\b/.test(s);
  if (e && typeof e === "object") {
    const obj = e as { name?: unknown; code?: unknown; message?: unknown };
    if (obj.name === "NotFound") return true;
    if (obj.code === 4001) return true;
    if (typeof obj.message === "string" && matches(obj.message)) return true;
    return false;
  }
  if (typeof e === "string") return matches(e);
  return false;
}

/**
 * Compare a desired attribute value against a raw IPA attribute value for the
 * `sync` reconcile diff. IPA stores attributes as (often single-element)
 * arrays; a desired value may be a scalar or an array. Both sides are
 * normalized to sorted string arrays and compared set-wise, so multi-valued
 * attributes (e.g. `mail`) are order-insensitive and `"John"` equals
 * `["John"]`.
 *
 * @param actualRaw The raw IPA value (from a parsed user's `raw` entry).
 * @param desired The desired value (scalar or array).
 * @returns `true` when the two represent the same attribute set.
 */
export function attrEquals(actualRaw: unknown, desired: unknown): boolean {
  const a = toStrArray(actualRaw).slice().sort();
  const d = toStrArray(desired).slice().sort();
  return a.length === d.length && a.every((x, i) => x === d[i]);
}

/** Minimal check context this model's pre-flight checks rely on. */
interface CheckContext {
  globalArgs: GlobalArgs;
  modelType: string;
  modelId: string;
  dataRepository: {
    getContent: (
      type: string,
      modelId: string,
      dataName: string,
      version?: number,
    ) => Promise<Uint8Array | null>;
  };
}

/** FreeIPA DNS management model definition. */
export const model = {
  type: "@shrug/freeipa/dns",
  version: "2026.07.12.1",
  description:
    "Manage FreeIPA DNS over the JSON-RPC API: zoneFind/zoneShow/recordFind read-only snapshots plus idempotent ensureZone/ensureForwardZone and a fan-out ensureRecords (add a list of records in one call), with confirm-guarded zoneDel/recordDel — each write carrying an audit trail. ensureRecords records partial applies honestly.",
  globalArguments: GlobalArgsSchema,
  resources: {
    "zones": {
      description: "Snapshot of zones matching a find (array of parsed rows)",
      schema: ZonesSchema,
      lifetime: "infinite",
      garbageCollection: 20,
    },
    "zone": {
      description:
        "Snapshot of a single DNS zone (parsed row + raw entry). Read by the zone-exists pre-flight check.",
      schema: ZoneSchema,
      lifetime: "infinite",
      garbageCollection: 20,
    },
    "records": {
      description:
        "Snapshot of a zone's records (array of parsed rows). `complete:false` marks a partial ensureRecords apply.",
      schema: RecordsSchema,
      lifetime: "infinite",
      garbageCollection: 20,
    },
    "record": {
      description:
        "Snapshot of a single DNS record (parsed row + raw entry). Written for the most recently ensured record; read by the record-exists pre-flight check.",
      schema: RecordSchema,
      lifetime: "infinite",
      garbageCollection: 20,
    },
    "attempt": {
      description:
        "Audit record of a mutation attempt (request + result/error + timestamp). Written on both success and failure.",
      schema: AttemptSchema,
      lifetime: "infinite",
      garbageCollection: 50,
    },
  },
  checks: {
    "zone-exists": {
      description:
        "Verify the most recently snapshotted zone still exists before a destructive zone delete.",
      labels: ["live"],
      appliesTo: ["zoneDel"],
      execute: async (
        context: CheckContext,
      ): Promise<{ pass: boolean; errors?: string[] }> => {
        // Pre-flight checks cannot see the method's arguments, so this verifies
        // against the last `zone` snapshot this model recorded. The method's own
        // confirm:true guard and IPA's NotFound error remain the per-zone
        // safeguards; this catches a stale-target delete against a live server.
        const bytes = await context.dataRepository.getContent(
          context.modelType,
          context.modelId,
          "zone",
        );
        if (!bytes) return { pass: true };
        let idnsname = "";
        try {
          const snap = JSON.parse(new TextDecoder().decode(bytes)) as {
            zone?: { idnsname?: string };
          };
          idnsname = snap.zone?.idnsname ?? "";
        } catch {
          return { pass: true };
        }
        if (!idnsname) return { pass: true };
        const client = await ipaLogin(context.globalArgs);
        try {
          await client.call("dnszone_show", [idnsname], {});
          return { pass: true };
        } catch (e) {
          return {
            pass: false,
            errors: [
              `zone "${idnsname}" not found on ${context.globalArgs.server}: ${
                e instanceof Error ? e.message : String(e)
              }`,
            ],
          };
        }
      },
    },
    "record-exists": {
      description:
        "Verify the most recently snapshotted record still exists before a destructive record delete.",
      labels: ["live"],
      appliesTo: ["recordDel"],
      execute: async (
        context: CheckContext,
      ): Promise<{ pass: boolean; errors?: string[] }> => {
        // Pre-flight checks cannot see the method's arguments, so this verifies
        // against the last `record` snapshot this model recorded. The method's
        // own confirm:true guard and IPA's NotFound error remain the per-record
        // safeguards; this catches a stale-target delete against a live server.
        const bytes = await context.dataRepository.getContent(
          context.modelType,
          context.modelId,
          "record",
        );
        if (!bytes) return { pass: true };
        let zone = "";
        let idnsname = "";
        try {
          const snap = JSON.parse(new TextDecoder().decode(bytes)) as {
            zone?: string;
            record?: { idnsname?: string };
          };
          zone = snap.zone ?? "";
          idnsname = snap.record?.idnsname ?? "";
        } catch {
          return { pass: true };
        }
        if (!zone || !idnsname) return { pass: true };
        const client = await ipaLogin(context.globalArgs);
        try {
          await client.call("dnsrecord_show", [zone, idnsname], {});
          return { pass: true };
        } catch (e) {
          return {
            pass: false,
            errors: [
              `record "${idnsname}" in zone "${zone}" not found on ${context.globalArgs.server}: ${
                e instanceof Error ? e.message : String(e)
              }`,
            ],
          };
        }
      },
    },
  },
  methods: {
    zoneFind: {
      description:
        "Snapshot zones matching an optional search criteria (read-only).",
      arguments: z.object({
        criteria: z
          .string()
          .optional()
          .describe("Free-text search; omit to list all zones"),
      }),
      execute: async (
        args: { criteria?: string },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;
        context.logger.info("Finding zones matching {criteria}", {
          criteria: args.criteria ?? "(all)",
        });
        const client = await ipaLogin(cfg);
        const res = await client.call("dnszone_find", [args.criteria ?? ""], {
          all: true,
        });
        const zones = ((res.result ?? []) as Array<Record<string, unknown>>)
          .map(parseZone);
        context.logger.info("Found {count} zones", { count: zones.length });

        const handle = await context.writeResource("zones", "zones", {
          server: cfg.server,
          zones,
          retrievedAt: new Date().toISOString(),
        });
        return { dataHandles: [handle] };
      },
    },
    zoneShow: {
      description: "Snapshot a single zone by name (read-only).",
      arguments: z.object({
        idnsname: z.string().describe("Zone name to fetch, e.g. example.com"),
      }),
      execute: async (
        args: { idnsname: string },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;
        context.logger.info("Showing zone {idnsname}", {
          idnsname: args.idnsname,
        });
        const client = await ipaLogin(cfg);
        const res = await client.call("dnszone_show", [args.idnsname], {
          all: true,
        });
        const zone = parseZone((res.result ?? {}) as Record<string, unknown>);
        context.logger.info("Retrieved zone {idnsname}", {
          idnsname: args.idnsname,
        });

        const handle = await context.writeResource("zone", "zone", {
          server: cfg.server,
          zone,
          retrievedAt: new Date().toISOString(),
        });
        return { dataHandles: [handle] };
      },
    },
    ensureZone: {
      description:
        "Idempotently ensure a DNS zone exists (dnszone_add). An existing zone (IPA DuplicateEntry) is a no-op success — the live entry is re-read via dnszone_show. Writes the zone state on success; audits both paths.",
      arguments: z.object({
        idnsname: z.string().describe("Zone name to create, e.g. example.com"),
        options: z
          .record(z.string(), z.unknown())
          .optional()
          .describe(
            "Extra raw IPA dnszone_add options (lowercase IPA option names), merged last",
          ),
      }),
      execute: async (
        args: { idnsname: string; options?: Record<string, unknown> },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;
        const client = await ipaLogin(cfg);
        const options: Record<string, unknown> = { ...(args.options ?? {}) };

        const { result, handle: attemptHandle } = await recordAttempt(
          context,
          args.idnsname,
          "dnszone_add",
          ["dnszone_add", "dnszone_show"],
          { idnsname: args.idnsname },
          async () => {
            try {
              return await client.call("dnszone_add", [args.idnsname], options);
            } catch (e) {
              // Idempotent create: an existing zone is a no-op success. Re-read
              // the live entry so the recorded response and the `zone` state
              // reflect reality. Non-duplicate errors still propagate.
              if (isDuplicateEntry(e)) {
                context.logger.info(
                  "dnszone_add: {idnsname} already exists, treating as no-op (idempotent)",
                  { idnsname: args.idnsname },
                );
                return await client.call("dnszone_show", [args.idnsname], {
                  all: true,
                });
              }
              throw e;
            }
          },
        );

        const zone = parseZone(
          (result.result ?? {}) as Record<string, unknown>,
        );
        const zoneHandle = await context.writeResource("zone", "zone", {
          server: cfg.server,
          zone,
          retrievedAt: new Date().toISOString(),
        });
        return { dataHandles: [attemptHandle, zoneHandle] };
      },
    },
    zoneDel: {
      description:
        "Delete a DNS zone (dnszone_del). Requires confirm:true; audits both paths.",
      arguments: z.object({
        idnsname: z.string().describe("Zone name to delete"),
        confirm: z
          .boolean()
          .describe("Must be true; a guard against accidental deletion"),
        idempotent: z
          .boolean()
          .optional()
          .default(false)
          .describe(
            "When true, an already-absent zone (IPA NotFound) is treated as success instead of failing. Default false preserves fail-on-missing. The confirm guard always applies.",
          ),
      }),
      execute: async (
        args: { idnsname: string; confirm: boolean; idempotent?: boolean },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        if (args.confirm !== true) {
          throw new Error(
            `Refusing to delete zone "${args.idnsname}": pass confirm:true to proceed`,
          );
        }
        const cfg = context.globalArgs;
        const client = await ipaLogin(cfg);
        const idempotent = args.idempotent ?? false;

        const { handle } = await recordAttempt(
          context,
          args.idnsname,
          "dnszone_del",
          ["dnszone_del"],
          { idnsname: args.idnsname, idempotent },
          async () => {
            try {
              return await client.call("dnszone_del", [args.idnsname], {});
            } catch (e) {
              // Idempotent delete: an already-gone zone is a no-op success.
              // Non-NotFound errors and the default (idempotent:false) path
              // still propagate so a real failure is never masked.
              if (idempotent && isNotFound(e)) {
                context.logger.info(
                  "dnszone_del: {idnsname} already absent, treating as no-op (idempotent)",
                  { idnsname: args.idnsname },
                );
                return { result: true, alreadyAbsent: true };
              }
              throw e;
            }
          },
        );
        return { dataHandles: [handle] };
      },
    },
    recordFind: {
      description:
        "Snapshot the records of a zone matching an optional criteria (read-only).",
      arguments: z.object({
        zone: z.string().describe("Zone to list records in, e.g. example.com"),
        criteria: z
          .string()
          .optional()
          .describe("Free-text search; omit to list all records in the zone"),
      }),
      execute: async (
        args: { zone: string; criteria?: string },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;
        context.logger.info(
          "Finding records in zone {zone} matching {criteria}",
          {
            zone: args.zone,
            criteria: args.criteria ?? "(all)",
          },
        );
        const client = await ipaLogin(cfg);
        const res = await client.call(
          "dnsrecord_find",
          [args.zone, args.criteria ?? ""],
          { all: true },
        );
        const records = ((res.result ?? []) as Array<Record<string, unknown>>)
          .map(parseRecord);
        context.logger.info("Found {count} records in {zone}", {
          count: records.length,
          zone: args.zone,
        });

        const handle = await context.writeResource("records", "records", {
          server: cfg.server,
          zone: args.zone,
          records,
          complete: true,
          retrievedAt: new Date().toISOString(),
        });
        return { dataHandles: [handle] };
      },
    },
    ensureRecords: {
      description:
        "Fan-out: idempotently ensure a LIST of records in one zone (dnsrecord_add per record, swallowing DuplicateEntry). One method run adds every record — do not loop N method runs (repo rule 6). Re-reads the zone via dnsrecord_find and snapshots the resulting `records` plus the last ensured `record`. On a partial failure the records that landed are persisted (complete:false) and the last ensured `record` snapshotted before the step is failed. Audits both paths.",
      arguments: z.object({
        zone: z.string().describe(
          "Zone to add the records into, e.g. example.com",
        ),
        records: z
          .array(RecordSpecSchema)
          .describe("The records to ensure; each { name, type, data, ttl? }"),
      }),
      execute: async (
        args: { zone: string; records: RecordSpec[] },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;

        // Live state tracked in the outer scope so that if a record fails hard
        // (a non-duplicate error) we can still persist the records that landed
        // (complete:false) and the last ensured record before failing the step
        // — a re-run is idempotent and reconciles the rest.
        let liveRecords: RecordRow[] = [];
        let lastEnsured: RecordRow | null = null;
        const persistRecords = (complete: boolean) =>
          context.writeResource("records", "records", {
            server: cfg.server,
            zone: args.zone,
            records: liveRecords,
            complete,
            retrievedAt: new Date().toISOString(),
          });
        const persistLast = () =>
          lastEnsured === null
            ? Promise.resolve({ name: "record" })
            : context.writeResource("record", "record", {
              server: cfg.server,
              zone: args.zone,
              record: lastEnsured,
              retrievedAt: new Date().toISOString(),
            });

        try {
          const { handle } = await recordAttempt(
            context,
            args.zone,
            "ensureRecords",
            ["dnsrecord_add", "dnsrecord_find"],
            {
              zone: args.zone,
              records: args.records.map((r) => ({
                name: r.name,
                type: r.type,
                data: r.data,
                ttl: r.ttl ?? null,
              })),
              count: args.records.length,
            },
            async () => {
              const client = await ipaLogin(cfg);
              const ensured: string[] = [];
              const failed: Array<
                { name: string; type: string; error: string }
              > = [];

              for (const r of args.records) {
                const options: Record<string, unknown> = {
                  [recordAttr(r.type)]: r.data,
                  ...(r.ttl !== undefined ? { dnsttl: r.ttl } : {}),
                };
                try {
                  await client.call(
                    "dnsrecord_add",
                    [args.zone, r.name],
                    options,
                  );
                  ensured.push(r.name);
                } catch (e) {
                  if (isDuplicateEntry(e)) {
                    ensured.push(r.name);
                  } else {
                    failed.push({
                      name: r.name,
                      type: r.type,
                      error: e instanceof Error ? e.message : String(e),
                    });
                  }
                }
              }

              // Re-read the zone so the snapshot reflects live state, including
              // any records that already existed (idempotent no-ops).
              const findRes = await client.call(
                "dnsrecord_find",
                [args.zone, ""],
                { all: true },
              );
              liveRecords =
                ((findRes.result ?? []) as Array<Record<string, unknown>>)
                  .map(parseRecord);
              if (ensured.length > 0) {
                const lastName = ensured[ensured.length - 1];
                lastEnsured = liveRecords.find((x) =>
                  x.idnsname === lastName
                ) ?? null;
              }

              if (failed.length > 0) {
                throw new Error(
                  `ensureRecords: ${failed.length} of ${args.records.length} record(s) failed in zone ${args.zone}: ` +
                    failed.map((f) => `${f.name}/${f.type}: ${f.error}`).join(
                      "; ",
                    ),
                );
              }

              return {
                zone: args.zone,
                ensured,
                failed,
                count: ensured.length,
              };
            },
          );

          // State resources, success-only.
          const recordsHandle = await persistRecords(true);
          const handles: Array<{ name: string }> = [handle, recordsHandle];
          if (lastEnsured !== null) handles.push(await persistLast());
          return { dataHandles: handles };
        } catch (e) {
          // Partial failure: some records may have landed. Persist that real
          // state (complete:false) and the last ensured record before
          // rethrowing so it is auditable and a re-run can reconcile the rest.
          // recordAttempt has already written the failure audit.
          await persistRecords(false);
          if (lastEnsured !== null) await persistLast();
          throw e;
        }
      },
    },
    recordDel: {
      description:
        "Delete a single DNS record's rdata (dnsrecord_del). Requires confirm:true; audits both paths.",
      arguments: z.object({
        zone: z.string().describe("Zone the record lives in, e.g. example.com"),
        name: z.string().describe("Record name relative to the zone, e.g. www"),
        type: z.string().describe("Record type, e.g. A, AAAA, CNAME"),
        data: z.string().describe("The rdata value to remove, e.g. 192.0.2.10"),
        confirm: z
          .boolean()
          .describe("Must be true; a guard against accidental deletion"),
        idempotent: z
          .boolean()
          .optional()
          .default(false)
          .describe(
            "When true, an already-absent record (IPA NotFound) is treated as success instead of failing. Default false preserves fail-on-missing. The confirm guard always applies.",
          ),
      }),
      execute: async (
        args: {
          zone: string;
          name: string;
          type: string;
          data: string;
          confirm: boolean;
          idempotent?: boolean;
        },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        if (args.confirm !== true) {
          throw new Error(
            `Refusing to delete record "${args.name}/${args.type}" in zone "${args.zone}": pass confirm:true to proceed`,
          );
        }
        const cfg = context.globalArgs;
        const client = await ipaLogin(cfg);
        const idempotent = args.idempotent ?? false;
        const options: Record<string, unknown> = {
          [recordAttr(args.type)]: args.data,
        };

        const { handle } = await recordAttempt(
          context,
          `${args.zone}/${args.name}`,
          "dnsrecord_del",
          ["dnsrecord_del"],
          {
            zone: args.zone,
            name: args.name,
            type: args.type,
            data: args.data,
            idempotent,
          },
          async () => {
            try {
              return await client.call(
                "dnsrecord_del",
                [args.zone, args.name],
                options,
              );
            } catch (e) {
              // Idempotent delete: an already-gone record is a no-op success.
              // Non-NotFound errors and the default (idempotent:false) path
              // still propagate so a real failure is never masked.
              if (idempotent && isNotFound(e)) {
                context.logger.info(
                  "dnsrecord_del: {name} in {zone} already absent, treating as no-op (idempotent)",
                  { name: args.name, zone: args.zone },
                );
                return { result: true, alreadyAbsent: true };
              }
              throw e;
            }
          },
        );
        return { dataHandles: [handle] };
      },
    },
    ensureForwardZone: {
      description:
        "Idempotently ensure a DNS forward zone exists (dnsforwardzone_add) with the given forwarders and policy. An existing forward zone (IPA DuplicateEntry) is a no-op success — the live entry is re-read via dnsforwardzone_show. Writes the zone state on success; audits both paths.",
      arguments: z.object({
        idnsname: z.string().describe("Forward zone name, e.g. example.com"),
        forwarders: z
          .array(z.string())
          .describe("Forwarder IP addresses (IPA `idnsforwarders`)"),
        forwardPolicy: z
          .string()
          .optional()
          .describe(
            "Forward policy (IPA `idnsforwardpolicy`): only/first/none",
          ),
      }),
      execute: async (
        args: {
          idnsname: string;
          forwarders: string[];
          forwardPolicy?: string;
        },
        context: ExecuteContext,
      ): Promise<ExecuteResult> => {
        const cfg = context.globalArgs;
        const client = await ipaLogin(cfg);
        const options: Record<string, unknown> = {
          idnsforwarders: args.forwarders,
          ...(args.forwardPolicy !== undefined
            ? { idnsforwardpolicy: args.forwardPolicy }
            : {}),
        };

        const { result, handle: attemptHandle } = await recordAttempt(
          context,
          args.idnsname,
          "dnsforwardzone_add",
          ["dnsforwardzone_add", "dnsforwardzone_show"],
          {
            idnsname: args.idnsname,
            forwarders: args.forwarders,
            forwardPolicy: args.forwardPolicy ?? null,
          },
          async () => {
            try {
              return await client.call(
                "dnsforwardzone_add",
                [args.idnsname],
                options,
              );
            } catch (e) {
              // Idempotent create: an existing forward zone is a no-op success.
              // Re-read the live entry so the recorded response and the `zone`
              // state reflect reality. Non-duplicate errors still propagate.
              if (isDuplicateEntry(e)) {
                context.logger.info(
                  "dnsforwardzone_add: {idnsname} already exists, treating as no-op (idempotent)",
                  { idnsname: args.idnsname },
                );
                return await client.call(
                  "dnsforwardzone_show",
                  [args.idnsname],
                  { all: true },
                );
              }
              throw e;
            }
          },
        );

        const zone = parseZone(
          (result.result ?? {}) as Record<string, unknown>,
        );
        const zoneHandle = await context.writeResource("zone", "zone", {
          server: cfg.server,
          zone,
          retrievedAt: new Date().toISOString(),
        });
        return { dataHandles: [attemptHandle, zoneHandle] };
      },
    },
  },
};
